...
// --------------------
   ...
   TH1F *RatioPolp = (TH1F*)histtvalue_All_Polp->Clone("RatioPolpPolm");

   
 

Double_t xl1=0.60, yl1=0.720, xl2=xl1+0.150, yl2=yl1+0.150;

TLegend *leg1 = new TLegend(xl1,yl1,xl2,yl2);
leg1->SetBorderSize(0);

leg1->AddEntry(RatioPolp,"(BH+TCS)^{+} / (BH+TCS)^{-}","L")->SetTextColor(4);
...
TCanvas* c1 = new TCanvas("c1","RatioPolpPolm", 10, 10, 900, 700);
   ...
   RatioPolp->GetYaxis()->SetTitle("(BH+TCS)^{+} / (BH+TCS)^{-}");
   ...
   RatioPolp->Divide(histtvalue_All_Polm);
...
c1->SaveAs("Ratio_Plp_to_Plm_Qp100.pdf");
//c1->SaveAs("Ratio_Plp_to_Plm_Qp100.C");
//c1->SaveAs("Ratio_Plp_to_Plm_Qp100.eps");
//c1->SaveAs("Ratio_Plp_to_Plm_Qp100.root");
